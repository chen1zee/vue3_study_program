class Jojo {
    constructor(opt) {
        ...
        this.data = {};
        this.methods = {};
        this.data = Jojo.data2InstanceData(opt.data(), this);
    ...
    static data2InstanceData(data, instance) {
        ...
        Object.entries(data).forEach(([key, val]) => {
            if (typeof val == "object" && !Array.isArray(val) && val !== null) { // object: {}
                ...
                // @ts-ignore
                temp[key] = Object.assign(Object.assign({}, Jojo.data2InstanceData(val, instance)), Jojo.createGSObj(val, instance));
                return;
            }
            // 普通类型 && array && null
            temp[key] = Jojo.createGSObj(val, instance);
        });
        return temp;
    }
    /**
