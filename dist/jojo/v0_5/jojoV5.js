...
export class JojoV5 {
    constructor(opt) {
         ...
         * */
        this.nowSetterDescsMap = new Map();
        /**
    ...
    /** 代理 data 层 g/setters */
    static createDataProxy(val, instance) {
        return new Proxy(val, {
            ...
            set(target, p, value, receiver) {
                ...
                // 处理 setters handle
                setMapMap(instance.nowSetterDescsMap, receiver, p, value);
            ...
            apply(target, _, argArray) {
                ...
                if (depWeakMap) { // 若有依赖列表 比对此次 所触发 proxy.key 是否命中依赖
                    ...
                    forEachMap(instance.nowSetterDescsMap, (keyVMap, proxyInstance) => {
                        ...
                        if (!proxyKeySet)
                            return true; // depWeakMap中无对应 依赖
                        // 遍历 nowSetterDesc 对应key
                        forEachMap(keyVMap, (_, key) => {
...
//# sourceMappingURL=jojoV5.js.map